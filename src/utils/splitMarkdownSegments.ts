export type TextSegment = { type: 'markdown'; content: string };
 ...
 */
export function preprocessReactComponents(text: string): string {
...
export function splitMarkdownSegments(markdown: string): MarkdownSegment[] {
  ...
  while ((match = CODE_FENCE_REGEX.exec(markdown)) !== null) {
    ...
    if (parts.length >= 3 && parts[0] === REACT_COMPONENT_SENTINEL) {
      ...
      segments.push({ type: 'component', componentId, props });
    }
