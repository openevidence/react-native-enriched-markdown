...
import { flattenSpoilerStyle, isStyleEqual } from "./styleUtils.js";
...
const DEFAULT_NORMALIZED_STYLE = Object.freeze({
  ...
  citation: {
    ...
    // 0 is a sentinel meaning "auto": native scales the chip to the
    // surrounding font, capped at line height. An explicit >0 value overrides.
    fontSize: 0,
    borderRadius: 999
  }
});
...
export const normalizeMarkdownStyle = style => {
  ...
  Object.keys(DEFAULT_NORMALIZED_STYLE).forEach(key => {
    const userValue = key === 'spoiler' ? flattenSpoilerStyle(style.spoiler) : style[key];
